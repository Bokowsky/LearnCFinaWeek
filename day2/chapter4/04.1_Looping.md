...
ColdFusion permits looping in CFScript using for loops and while
loops.
...
example allows you to break down a string into individual words by specifying
...
(struct) one iteration per key. Using associative array notation, you
...
looping as normal. You can use the `cfcontinue` tag to perform
this logic. In ``cfscript`` you can use the `continue` keyword.
